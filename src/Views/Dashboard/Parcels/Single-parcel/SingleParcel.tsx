  ...
  const apiUrl = process.env.NEXT_PUBLIC_PORT; 
	...
	return (
		<Layout>
			<div className="w-full mx-auto flex flex-col gap-6 overflow-hidden">
				...
				<div className="grid grid-cols-3 gap-4 max-xl:grid-cols-2 max-md:grid-cols-1">
					<StatCard
						...
						value={singleParcel?.status ?? "N/A"}
						icon={<Package className="text-textColorLight dark:text-textColorDark" />}
						isLoading={isLoading}
					/>
					<StatCard
						title="Parcel Time"
						value={formatDateOnly(singleParcel?.createdAt ?? "N/A")}
						icon={<Calendar className="text-textColorLight dark:text-textColorDark" />}
						isLoading={isLoading}
					/>
					<StatCard
						title="Package Value"
						value={singleParcel?.transaction?.totalPrice ? `${singleParcel?.transaction?.totalPrice}` : "N/A"}
						icon={<HandCoins className="text-textColorLight dark:text-textColorDark" />}
						isLoading={isLoading}
					/>
				</div>
				<div
					className="bg-[#F3F5FA] dark:bg-cardBG rounded-sm border-2 dark:border-textColorDark w-full border-transparent flex flex-col gap-4"
				...
				>
					<div className="w-full flex justify-between items-center max-sm:flex-col max-sm:items-start max-sm:gap-2">
						<div className="flex items-center gap-2">
							...
							<StatusBox
								value={singleParcel?.status == "DELIVERED/PICKED UP" ? "Paid" : "In-Progress"}
								color={singleParcel?.status == "DELIVERED/PICKED UP" ? "green" : "yellow"}
								isLoading={isLoading}
							/>
						</div>
						<div className="flex items-center gap-2">
					...
					<div className="flex gap-4 flex-wrap">
						{allStatus.map((item, index) => {
							...
							return (
								<div
									key={`status${index}`}
									className="flex flex-col gap-1 w-[12rem] max-lg:w-[22.5%] max-md:w-[30%] max-sm:w-full"
								>
				...
				>
					<div className="flex items-center gap-2">
						...
						<StatusBox
							value={singleParcel?.transaction?.paymentStatus == "PAYMENT VALIDATED" ? "Paid" : "Pending"}
							color={singleParcel?.transaction?.paymentStatus == "PAYMENT VALIDATED" ? "green" : "yellow"}
						/>
					</div>
					<div className="flex gap-4 flex-wrap">
				...
				>
					<div
						className="flex items-center gap-4 border-b dark:border-b-textColorDark border-b-transparent w-full"
						style={{paddingLeft: "0.75rem", paddingRight: "0.75rem"}}
					>
						{tabs.map((tab, index) => (
							<p
								key={`tabs${index}`}
								className={`cursor-pointer border-b-2 ${activeTab === tab ? "text-[#155dfc] border-b-[#155dfc]" : "border-b-transparent"}`}
					...
					{activeTab == "Parcel Details" ? (
						<div
							className="grid grid-cols-4 gap-4 max-2xl:grid-cols-3 max-xl:grid-cols-2 max-sm:grid-cols-1 max-sm:gap-2"
							style={{paddingLeft: "0.75rem", paddingRight: "0.75rem"}}
						>
										...
										<div className="flex items-center gap-1">
											{Array.from({length: 3}).map((_, index) => (
												<Image
													...
													width={150}
													height={50}
													className="w-[32.5%] h-[70px] object-cover rounded-md"
												/>
											))}
										</div>
									</div>
										...
										<div className="flex items-center gap-1 overflow-x-auto">
											{singleParcel?.packagePicture?.map((item, index) => (
												<Image
													...
													width={150}
													height={50}
													className="w-[32.5%] h-[70px] object-cover rounded-md cursor-pointer"
						...
						<div
							className="flex flex-col gap-2 w-[20rem] max-md:w-full"
							style={{paddingLeft: "0.75rem", paddingRight: "0.75rem"}}
						>
			...
			>
				<UpdateStatus
					agencyID={singleParcel?.agency?._id ?? ""}
