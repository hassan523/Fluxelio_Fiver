...
const UpdateStatus = ({ agencyID, UpdateStatusData, setterFunction }: {agencyID:string, UpdateStatusData: { isOpen: boolean; data: any }; setterFunction: Dispatch<SetStateAction<{ isOpen: boolean; data: any }>> }) => {
    ...
    const { parcelStatus, partialAmount, paymentStatus } = parcelData;
    ...
    const { handleUpdateParcel, isError, isLoading: updateParcelLoading } = useUpdateParcel();
    ...
    const handleUpdateStatus = async () => {
        ...
        await handleUpdateParcel({
            parcelID: data?._id,
            agencyID: agencyID,
            officeID: data?.departure?._id,
            Token,
            updatedBy: _id,
            data: {
                status: parcelStatus || data?.status,
                paymentStatus: paymentStatus || data?.transaction?.paymentStatus,
            ...
            <form className="flex flex-col gap-2">
                            ...
                            {statusArray?.map((Status) => (
                                <React.Fragment key={Status}>
                                    <DropdownMenuLabel
                                        ...
                                        onClick={() => {
                                            setParcelData((prev) => ({ ...prev, parcelStatus: Status }));
                                        }}
                                    >
                                        {Status}
                                    </DropdownMenuLabel>
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger className="dark:bg-layoutDarkBG bg-white px-2.5 py-2 rounded-sm border dark:border-white border-gray-500  outline-none">
                            <span className={`flex justify-between items-center gap-6 w-full dark:text-white text-textColorLight cursor-pointer`}>
                                <span className="w-full">{paymentStatus || data?.transaction?.paymentStatus}</span>
                                <ChevronDown size={16} />
                            </span>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent>
                            {paymentArray?.map((Status) => (
                                <React.Fragment key={Status}>
                                    <DropdownMenuLabel
                                        className="cursor-pointer"
                                        onClick={() => {
                                            setParcelData((prev) => ({ ...prev, paymentStatus: Status }));
                                        }}
                                    >
                                        {Status}
                                    </DropdownMenuLabel>
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <Field
                        ...
                        onChange={(e: any) => {
                            setParcelData((prev) => ({ ...prev, partialAmount: e.target.value }));
                        }}
                        name="Partial Amount"
